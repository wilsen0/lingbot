<template>
  ...
  >
      ...
      <slot name="art">
        <svg viewBox="0 0 140 110" class="ui-empty__svg" aria-hidden="true">
          ...
          <g fill="url(#e_petal)">
            ...
            <ellipse cx="48" cy="104" rx="4" ry="2.1" transform="rotate(-15 48 104)" opacity="0.8" />
            <ellipse cx="118" cy="94" rx="5.5" ry="2.8" transform="rotate(32 118 94)" opacity="0.9" />
            <ellipse cx="100" cy="102" rx="3.5" ry="1.9" transform="rotate(-10 100 102)" opacity="0.8" />
          </g>
        </svg>
      </slot>
    </div>
    ...
    <h3 v-if="variant === 'full' || $slots.title" class="ui-empty__title font-display">
      <slot name="title">此处空空</slot>
    </h3>

    <p v-if="$slots.default" class="ui-empty__hint" :class="{ 'ui-empty__hint--solo': variant === 'compact' && !$slots.title }">
      <slot />
    </p>
