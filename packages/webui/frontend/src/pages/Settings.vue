  ...
  <section class="set">
    ...
    <section class="set__block">
      <header class="set__head">
        <h2 class="set__title font-display">在册</h2>
        <p class="set__hint">登记在册的 bot · 平台 · 当前是否在线</p>
      </header>
      ...
      <ul v-if="bots.length" class="set__bots">
        ...
        >
          ...
          <button
            ...
            :aria-label="`重唤 ${b.name || b.id}（热加载规则）`"
            @click="reload(b.id)"
          >
            <DecoBellLoader v-if="reloading === b.id" size="sm" />
            <span v-else class="font-display">重唤</span>
          </button>
        </li>
      </ul>
      <UiEmptyState v-else variant="compact">尚无 bot 登记</UiEmptyState>

      <p v-if="auth.profile?.role" class="set__role-line">
        <span class="set__role-mark" aria-hidden="true">·</span>
        <span>掌门 — 你是 <strong>{{ roleLabel }}</strong></span>
      </p>
    </section>
    ...
    <section class="set__block">
      <header class="set__head">
        <h2 class="set__title font-display">时辰</h2>
        <p class="set__hint">背景的天色 · 随系统、白日、夜色三选一</p>
      </header>

      <div class="set__themes" role="radiogroup" aria-label="时辰">
        <button
          v-for="t in themes"
          :key="t.value"
          role="radio"
    ...
    <section class="set__block">
      <header class="set__head">
        <h2 class="set__title font-display">装饰</h2>
        <p class="set__hint">背景里的花瓣 · 雾色 · 萤光浓度</p>
      </header>

      <div class="set__decors" role="radiogroup" aria-label="装饰浓度">
        <button
          v-for="d in decors"
          :key="d.value"
          role="radio"
          ...
          <span class="decor-card__art" aria-hidden="true">
            <svg viewBox="0 0 40 28" fill="none">
              ...
              <g v-if="d.value !== 'off'" fill="rgb(var(--color-petal))">
                ...
                <ellipse v-if="d.value === 'full'" cx="22" cy="8" rx="3" ry="1.6" transform="rotate(30 22 8)" />
                <ellipse v-if="d.value === 'full'" cx="30" cy="20" rx="3" ry="1.6" transform="rotate(-20 30 20)" />
                <ellipse v-if="d.value === 'full'" cx="36" cy="12" rx="3" ry="1.6" transform="rotate(15 36 12)" />
              </g>
              <path
    ...
    <section class="set__block">
      <header class="set__head">
        <h2 class="set__title font-display">铃响 · 震感</h2>
        <p class="set__hint">事毕一响铃；移动端轻震 15ms</p>
      </header>

      <div class="set__switch-row">
        <div class="set__switch-info">
          <p class="set__switch-name font-display">铃响</p>
          <p class="set__switch-sub">回复时一缕铃音</p>
        </div>
        <UiSwitch
      ...
      <div class="set__switch-row">
        <div class="set__switch-info">
          <p class="set__switch-name font-display">震感</p>
    ...
    <section class="set__about">
      ...
      <button class="set__logout tap" @click="onLogout">
        ...
        <span class="set__logout-label font-display">解&thinsp;缘</span>
...
const roleLabel = computed(() => {
  switch (auth.profile?.role) {
    case "superadmin": return "掌门";
    case "bot_admin": return "执事";
    case "readonly": return "客卿";
    default: return "";
  }
});

const themes: { value: ThemeMode; label: string; hint: string; aria: string }[] = [
  { value: "auto",  label: "随系", hint: "跟随系统",  aria: "随系统时辰" },
  { value: "light", label: "白日", hint: "晨光樱粉",  aria: "白日主题" },
  { value: "dark",  label: "夜色", hint: "薰衣紫天",  aria: "夜色主题" },
];
const decors: { value: DecorLevel; label: string; hint: string; aria: string }[] = [
  { value: "full",   label: "满目", hint: "瓣多 · 萤密", aria: "满目装饰" },
  { value: "subtle", label: "淡雅", hint: "瓣少 · 静谧", aria: "淡雅装饰" },
  { value: "off",    label: "静寂", hint: "无装饰",      aria: "关闭装饰" },
];
...
async function reload(botId: string) {
  ...
  try {
    ...
    if (!applied) {
      ...
      toast.success("已重唤", `${n} 处理器 / ${files} 文件`);
      // 让卡片金光脉冲一下
      pulsingId.value = botId;
      stage.ringBell();
      setTimeout(() => { pulsingId.value = null; }, 1200);
    }
  } catch (e) {
...
async function onLogout() {
  ...
  if (!ok) return;
  try { if (auth.refreshToken) await apiLogout(auth.refreshToken); }
  catch { /* logout API 失败也照样清本地 */ }
  finally {
    auth.clear();
...
.set {
  ...
  /* 入场由 AppShell 的 fade-up router-transition 接管, 这里不再二次淡入 */
}

/* ===== block ===== */
.set__block { display: flex; flex-direction: column; gap: 12px; }
.set__head { display: flex; flex-direction: column; gap: 4px; }
.set__title {
...
.set__role-mark { color: rgb(var(--color-thread)); font-size: 16px; line-height: 1; }
.set__note {
  font-size: 11px;
  color: rgb(var(--color-ink-soft));
  letter-spacing: var(--track-fn);
...
.set__bot {
  ...
  min-height: 60px;
  position: relative;
  transition: box-shadow var(--dur-slow) ease, background var(--dur-slow) ease;
  border-radius: var(--radius-seal);
}
.set__bot.is-pulsing {
  background: rgb(var(--color-bell) / 0.12);
  box-shadow: 0 0 0 1px rgb(var(--color-bell) / .35), 0 0 24px rgb(var(--color-bell) / .25);
}
...
.set__bot-dot.off { background: rgb(var(--color-ink-soft) / 0.5); }
.set__bot-info { flex: 1; min-width: 0; }
.set__bot-name {
  font-size: 17px;
...
.set__bot-action {
  ...
  border-radius: var(--radius-seal);
  transition: color var(--dur-fast) ease, transform var(--dur-tap) var(--ease-tap), background var(--dur-base) ease;
  display: inline-flex;
  align-items: center;
...
.set__bot-action:hover:not(:disabled) {
  ...
  background: rgb(var(--color-thread) / .08);
}
.set__bot-action:active:not(:disabled) { transform: scale(0.94); }
.set__bot-action:disabled { opacity: 0.5; cursor: not-allowed; }
...
.theme-card {
  ...
  background: rgb(var(--color-bg-veil) / .42);
  border: 0;
  cursor: pointer;
  border-radius: var(--radius-paper);
  transition:
...
.theme-card:hover { background: rgb(var(--color-bg-veil) / .65); }
.theme-card:active { transform: scale(0.97); }
.theme-card[aria-checked="true"] {
  background: rgb(var(--color-bg-veil) / .85);
  box-shadow: 0 0 0 1px rgb(var(--color-thread) / .45), 0 12px 28px rgb(var(--color-sorrow) / .14);
}
/*
...
.decor-card::after {
  ...
  background: linear-gradient(
    to right,
    transparent,
    rgb(var(--color-sorrow)),
    transparent
  );
  filter: drop-shadow(0 1px 2px rgb(var(--color-sorrow) / .4));
  border-radius: 2px;
  transform: scaleX(0);
...
.theme-card__sky svg { width: 100%; height: 100%; }
.theme-card__label {
  font-size: 16px;
...
.decor-card:hover { background: rgb(var(--color-bg-veil) / .65); }
.decor-card:active { transform: scale(0.97); }
...
.decor-card__art svg { width: 100%; height: 100%; }
.decor-card__label {
  font-size: 16px;
  letter-spacing: 0.22em;
  color: rgb(var(--color-ink));
...
.set__switch-row {
  ...
  gap: 14px;
  padding: 14px 16px;
  background: rgb(var(--color-bg-veil) / .5);
  backdrop-filter: blur(14px);
  border-radius: var(--radius-paper);
  box-shadow: 0 8px 22px rgb(0 0 0 / .14);
  margin-top: 4px;
}
.set__switch-info { flex: 1; min-width: 0; }
.set__switch-name {
  font-size: 16px;
  letter-spacing: var(--track-meta);
  color: rgb(var(--color-ink));
...
.set__logout:hover {
  color: rgb(var(--color-sorrow));
  background: rgb(var(--color-alert) / .08);
}
.set__logout:active { transform: scale(0.96); }
.set__logout-mark {
  font-size: 18px;
  line-height: 1;
...
@media (max-width: 420px) {
  ...
  .theme-card,
  .decor-card { padding: 12px 4px 10px; min-height: 88px; }
  .theme-card__label,
  .decor-card__label { font-size: 14px; letter-spacing: 0.18em; }
}
</style>
