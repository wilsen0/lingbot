...
- [x] 2. 定义统一事件/动作模型
    - [x] 2.1 `ap_core.events.Event / Action / Segment / Scope / User` 的 pydantic 模型
    - [x] 2.2 Segment 枚举与编解码辅助（from_onebot_msg / to_onebot_msg 预留）
    - [x] 2.3 `EventBus`：异步发布订阅，支持优先级
    - _Requirements: US-P3, US-P4_

- [ ] 3. KV 存储（SQLite 实现）
    - [ ] 3.1 `KVStore` 接口 + SQLite 实现（含 `read/write/delete/rank`）
    - [ ] 3.2 迁移工具：自动建表、wal 模式、事务封装
    - [ ] 3.3 单测：读写、并发、排行榜语义与老 `$排行榜$` 对齐
    - _Requirements: US-S1, NFR-5_

- [ ] 4. 工具注册表（Tool Registry）
    - [ ] 4.1 `@tool` 装饰器：Python 名 / DSL 名 / JSON schema / safe 标志
    - [ ] 4.2 运行时三视图（python call / dsl dispatch / llm schema）
    - [ ] 4.3 标准工具：`read_kv / write_kv / delete_kv / rank_kv / http_get / http_post / random_int / regex_match / replace`
    - _Requirements: US-M3_

- [ ] 5. CLI 适配器（最简）
    - [ ] 5.1 `ap repl`：stdin 作消息，stdout 作回复
    - [ ] 5.2 支持 `@用户` / `%群号%` 的手动注入，便于调试
    - _Requirements: S-4_

- [ ] 6. DSL v0：词法与语法解析器
    - [ ] 6.1 词法器：关键字、字符串、数字、标识符、注释、`$...$`、`%...%`、`[...]`、`±img=...±`
    - [ ] 6.2 语法器：按 EBNF 生成 AST（采用 `lark` 或手写 PEG）
    - [ ] 6.3 错误定位：行列号、上下文片段
    - [ ] 6.4 单测：对 `dicpro.txt` 里 30+ 条代表性 handler 正确生成 AST
    - _Requirements: US-D2, US-D3, US-D4_

- [ ] 7. DSL v0：解释器 / 虚拟机
    - [ ] 7.1 handler 作用域 + 局部变量 + 内置变量 + KV 读穿透
    - [ ] 7.2 `如果/如果尾/正则/返回/完成/:label/$jump$`
    - [ ] 7.3 算术表达式求值（整型/浮点/字符串拼接歧义处理）
    - [ ] 7.4 工具调用派发（经 Tool Registry）
    - [ ] 7.5 沙箱：超时、步数、输出段数
    - [ ] 7.6 单测：与 QRDic 黄金样例比对
    - _Requirements: US-D3, US-D5, NFR-4_

- [ ] 8. 调度器
    - [ ] 8.1 `scheduler.delay(ms, handler, args)` 内存实现
    - [ ] 8.2 持久化：重启恢复未触发任务
    - [ ] 8.3 `$调用 ms handler$` DSL 入口
    - _Requirements: US-T1, US-T3_

- [ ] 9. 最小 OneBot 适配器
    - [ ] 9.1 反向 WS 客户端，`message / notice / request` 事件 → `Event`
    - [ ] 9.2 `Action` → OneBot API 调用（`send_msg / delete_msg / set_group_ban / set_group_special_title`）
    - [ ] 9.3 CQ / message array 双向映射（text/image/at/reply/face/poke）
    - [ ] 9.4 能力矩阵实现 + 不支持能力的降级
    - _Requirements: US-P1, US-P3, US-P4_

- [ ] 10. 配置系统
    - [ ] 10.1 `bot.yaml` 加载 + ${ENV} 展开
    - [ ] 10.2 `.env` + secrets 加载
    - [ ] 10.3 HTTP 白名单、沙箱参数、主群/管理员从配置注入
    - _Requirements: US-X3, MIG-4_

- [ ] 11. 端到端 smoke：P0 验收
    - [ ] 11.1 CLI 适配器跑通"打卡 → 写灵玉 → 查看背包"三条规则
    - [ ] 11.2 OneBot mock 跑通同样三条
    - _Requirements: S-1, S-2_

## 阶段 P1 — 迁移器 + 完整指令兼容

- [ ] 12. 迁移器 `ap migrate qrdic`
    - [ ] 12.1 分段切 handler；识别 `[内部]`；识别 `&&` 配置/注释
    - [ ] 12.2 `$BSH 图文.java imagettftext$` → `$图文$` 重写
    - [ ] 12.3 硬编码号码 → 占位 + 配置；`/storage/emulated/...` 路径 → 资源引用
    - [ ] 12.4 Properties → KV（含 Unicode 转义解码，忽略 `.bak`）
    - [ ] 12.5 输出 `migration_report.md`：无法迁移段、TODO、警告
    - _Requirements: MIG-1, MIG-2, MIG-3, MIG-4, S-2_

- [ ] 13. 标准工具库补齐
    - [ ] 13.1 `JSON 长度/获取/添加`、`URLEncoder/Decoder`、`Base64Decoder`、`HexEncoder/Decoder`、`UnicodeDecoder`
    - [ ] 13.2 `替换 / 正则 / 概率随机`
    - [ ] 13.3 `群昵称 / 群头衔 / 获取群成员 / 获取消息`（通过适配器 RPC）
    - [ ] 13.4 `图文`（Pillow，中文 TTF，复刻 `图文.java` 行为）
    - [ ] 13.5 `全局变量 / 取变量`（进程内 + 可选持久）
    - _Requirements: US-D3, MIG-3_

- [ ] 14. `ap lint`
    - [ ] 14.1 语法错误 / 未使用变量 / 不可达代码检测
    - [ ] 14.2 危险工具使用（`$删除$`、`$访问$`、`$撤回$`）在缺少权限声明时报错
    - [ ] 14.3 触发器正则冲突检测（多个 handler 会同时匹配）
...
- [ ] 17. Agent 定义与运行时
    - [ ] 17.1 `agent.yaml` schema：provider/model/system/tools/memory/triggers/guardrails
    - [ ] 17.2 ReAct 工具调用循环 + 守护（超时、最大工具链、token 上限）
    - [ ] 17.3 触发器：`mention / dm / keyword / fallback / always`
    - [ ] 17.4 流式回复到适配器
    - _Requirements: US-A1, US-A3, US-A5, US-A6_

- [ ] 18. 记忆
    - [x] 18.1 短期滚动窗口
    - [ ] 18.2 长期向量库（sqlite-vss 默认实现）
    - [x] 18.3 记忆 namespace：`bot × user × scope`
    - _Requirements: US-A4_

- [ ] 19. DSL ↔ Agent 桥
    - [x] 19.1 `$agent 调用 name input$` / `$agent 流式 name input$`
    - [ ] 19.2 DSL 处理器可通过 `expose_as_tool: true` 注册成 LLM 工具
    - [ ] 19.3 工具调用权限传递（Agent 调 DSL 时继承 Agent 的用户身份）
...
- [ ] 21. 观测
    - [ ] 21.1 Prometheus 指标
    - [ ] 21.2 审计日志表 + 检索 CLI
    - [ ] 21.3 最小 Web UI（事件流 / KV 浏览器 / 规则命中）
    - _Requirements: US-O1, US-O2_

- [ ] 22. 热加载
    - [ ] 22.1 监听 `rules/**/*.ap` 与 `agents/*.yaml` 变更 → 平滑替换
...
- [ ] 26. 文档与示例
    ...
    - [ ] 26.3 迁移指南（QRDic → agent-platform）
    - [ ] 26.4 `bot/` 完整复刻示例
...
- [ ] D-1 项目正式名（`AgentDic` / `Kitsune` / `FoxFly` / 其他）
- [ ] D-2 规则文件扩展名（`.ap` / `.dic` / `.fox`）
